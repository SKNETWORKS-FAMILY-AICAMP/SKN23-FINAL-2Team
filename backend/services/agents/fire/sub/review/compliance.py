...
_FIRE_VIOLATION_TYPES_STR = (
    ...
    "access_blocked | material_error | coverage_error | other_violation"
)
...
각 설비(element)는 다음 필드를 가집니다:
  ...
  - fire_topology: 설비 유형별 최근접 거리 분석 결과
    ...
    - fire_topology.extinguisher.coverage_gaps       : 바닥 커버리지 공백 후보 (직선거리 20m 초과 지점)
    - 소화기 20m 검토는 소화기끼리 거리(spacing)가 아니라 coverage_gaps만 근거로 사용
...
3. 간격 검토: 각 설비의 간격 위반은 반드시 fire_topology.<설비유형>.violation_candidates 또는
   ...
   violation_candidates가 비어 있으면 해당 설비의 간격 위반을 보고하지 마십시오.
3-1. coverage_error 처리: [코드 사전 계산 결과 — 재판단 금지] 힌트에 coverage_error 항목이 있으면,
     해당 위반의 distance_mm, x, y, nearest_extinguisher_id, fire_category 필드를 그대로 출력에 유지하십시오.
     violation_type은 반드시 "coverage_error"로 고정합니다.
3-2. 소화기 pairwise spacing 금지: "소화기 A와 B 간 거리" 형식의 spacing_error는 보고하지 마십시오.
     소화기 위반은 coverage_error 후보가 있을 때만 보고하십시오.
    ...
    async def check_compliance_parsed(
        ...
        if (
            ...
            return await self.check_compliance(
                target_id, spec_context, layout_str,
        ...
        async def _one(k: int, el_chunk: list) -> list:
            async with sem:
                ...
                # candidates_hint는 모든 청크에 전달: 확정 위반 후보가 어느 청크에 있을지 알 수 없음
                extra = candidates_hint + chunk_suffix
                return await self.check_compliance(
                    t_id, spec_context, sub_str, extra_user_suffix=extra
                )
