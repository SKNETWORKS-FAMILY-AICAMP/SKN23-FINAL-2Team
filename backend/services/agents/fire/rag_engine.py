...
class FireRagEngine:
    ...
    async def retrieve(
        ...
        queries = self._build_queries(main_query, extra_queries, include_default_queries)
        search_domains = self._resolve_domains(domain)
        doc_name_filter = target_doc if strict_target_doc and target_doc else None
        log.info(
            "[FireRagEngine] queries=%d개, domains=%s, final_limit=%d, target_doc=%r strict=%s",
            len(queries),
            search_domains,
            final_limit,
            target_doc,
        ...
        if strict_target_doc and target_doc and not any(results_per_query):
            log.warning(
                "[FireRagEngine] strict target_doc 검색 결과 없음 -> doc_name 필터 완화 재검색 target_doc=%r",
                target_doc,
            )
            relaxed_tasks = [
                self._search_one(
    ...
    def _build_queries(
        ...
        sub = FIRE_SUB_QUERIES if include_default_queries else []
        all_queries = [main_query] + sub + (extra_queries or [])
        seen: set[str] = set()
        unique = []
    ...
    def _resolve_domains(domain: str | None) -> list[str]:
        d = (domain or "fire").strip()
        if d in ("fire", "소방", "firefighting"):
            return list(_FIRE_DB_DOMAINS)
        return [d, *[x for x in _FIRE_DB_DOMAINS if x != d]]
    ...
    def _to_dict(chunk) -> dict[str, Any]:
        ...
        content = FireRagEngine._get_context_content(chunk)
        return {
            "source":      "temp" if is_temp else "permanent",
            "id":          getattr(chunk, "id", None),
            "document_id": getattr(chunk, "temp_document_id", None) if is_temp else getattr(chunk, "document_id", None),
