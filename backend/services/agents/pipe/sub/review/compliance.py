...
from backend.services.agents.pipe.sub.drawing_type import pipe_drawing_type_from_text
from backend.services import llm_service

# LLM 호출 안정화 — 본 파일 단위에서 응답 견고성 보강
_LLM_CALL_TIMEOUT_SEC = 90.0
_LLM_CALL_MAX_ATTEMPTS = 2  # 타임아웃·일시적 실패 시 1회 재시도
_LLM_CALL_RETRY_BACKOFF_SEC = 1.5
...
def _repair_and_parse_json(raw: str) -> dict | None:
    """비-JSON·부분 손상 응답을 최선 노력으로 dict 로 복구합니다.

    시도 순서:
      1. 직접 ``json.loads``
      2. 마크다운 펜스(```json ... ```) 제거 후 재시도
      3. 첫 ``{`` ~ 마지막 ``}`` 추출 후 재시도
    ...
    for candidate in list(candidates):
        candidates.append(_TRAILING_COMMA_RE.sub(r"\1", candidate))
...
async def _call_llm_with_retry(
    *,
    ...
    timeout_sec: float = _LLM_CALL_TIMEOUT_SEC,
    max_attempts: int = _LLM_CALL_MAX_ATTEMPTS,
):
    """``llm_service.generate_answer`` 호출에 timeout + 짧은 재시도를 둡니다.

    sLLM 큐 밀림이나 일시적 connection reset 에 대해 한 차례 더 시도해 신뢰성을 높입니다.
    재시도 후에도 실패하면 마지막 예외를 그대로 전파합니다.
    """
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            return await asyncio.wait_for(
                llm_service.generate_answer(**kwargs),
                timeout=timeout_sec,
            )
        except asyncio.TimeoutError as exc:
            last_exc = exc
            logging.warning(
                "[ComplianceAgent] LLM timeout attempt=%d/%d timeout=%.1fs",
                attempt,
                max_attempts,
                timeout_sec,
            )
            ...
            last_exc = exc
            logging.warning(
                "[ComplianceAgent] LLM call error attempt=%d/%d: %s",
                attempt,
                max_attempts,
                exc,
            )
        if attempt < max_attempts:
            await asyncio.sleep(_LLM_CALL_RETRY_BACKOFF_SEC * attempt)
    if last_exc is not None:
        raise last_exc
    raise RuntimeError("LLM call failed without recorded exception")
...
_ELEMENT_WHITELIST: frozenset[str] = frozenset({
    # 기본 식별
    "handle", "id", "type", "raw_type", "layer", "name", "text",
    "layer_role", "source_layer_role", "metadata_role", "geometry_role",
    "connectivity_role", "flag_for_piping_agent", "source_attributes",
    # 배관 핵심 속성
...
def _has_pipe_evidence(slim: dict) -> bool:
    if slim.get("flag_for_piping_agent"):
        return True
    if str(slim.get("layer_role") or "").lower() == "mep":
        return True
    for key in (
        "diameter_mm", "pressure_mpa", "slope_pct", "flow_rate_m3h",
        "temp_c", "velocity_ms", "hanger_spacing_mm", "elevation_mm",
    ):
        value = slim.get(key)
        ...
        if value not in (None, "", 0, 0.0):
            return True
    material = str(slim.get("material") or "").strip().upper()
    return bool(material and material != "UNKNOWN")
...
def _slim_element(el: dict) -> dict:
    ...
    if (
        ent_type in ("LINE", "LWPOLYLINE")
        and not slim.get("text")
        and not slim.get("name")
        and not _has_pipe_evidence(slim)
    ):
        return {}
    ...
    if "pipe_topology" in slimmed:
        ...
        if isinstance(topo, dict) and "pipe_runs" in topo:
            ...
            slimmed["pipe_topology"] = {
                ...
                "connection_overshoots": (topo.get("connection_overshoots") or [])[:30],
...
[보안/신뢰 경계]
  ...
  - raw_type이 TEXT/MTEXT/MLEADER/LEADER인 element는 배관 주석 문맥입니다. 주석 텍스트 자체를
    가스계량기·밸브·배관 본체로 간주하여 재질/규격/이격거리 위반을 만들지 마십시오.

[RAG 문맥 사용 규칙]
  - [시방서 규정]에는 `[RAG n] source=... doc=... section=... hits=... doc_score=...` 헤더가 붙을 수 있습니다.
    hits가 큰 청크는 여러 검색 질의에서 반복 검색된 근거이므로 우선 참고하되, 본문 내용과 도면 데이터가
    직접 일치할 때만 위반으로 작성하십시오.
  - doc_score는 문서명이 현재 도면에 적합한지 별도 판독한 점수입니다. 낮은 doc_score 문서는
    보조 참고로만 사용하고, 단독 근거로 확정 위반을 만들지 마십시오.
  - source=temp는 사용자가 업로드한 시방서 청크일 수 있으므로 동일한 구체성의 permanent 근거보다 우선하십시오.
  - `[배관 도면검토 내장 fallback 기준]` 문맥은 RAG 검색이 모두 실패한 경우의 보수적 기본 기준입니다.
    이 경우 수치 기준을 새로 만들지 말고, 도면 데이터에 명확한 연결/충돌/누락 근거가 있는 항목만
    낮은 신뢰도 또는 수동 검토 성격으로 보고하십시오.

[건축·구조 참조 (arch_elements, 선택)]
  JSON에 `arch_elements` 배열이 있으면, 이는 A-/S- 레이어 등에서 분리된 건축·구조 참조입니다.
  arch_elements가 없을 때만 긴 축방향 선분에서 추정한 proxy_wall이 대용으로 주입될 수 있습니다.
...
[추가 공간 분석 필드]
  - pipe_topology   : pipe_runs(배관 경로 목록, handles·total_length_mm·connected_blocks·material·diameter_mm)
                      ...
                      virtual_connections는 TEXT/MTEXT 주석(G, GAS, 20A, DN, D/S, 100 등)이 도면상 끊긴
                      표시 간격 안에 있는 컨텍스트 정보입니다. 물리 연결로 간주하지 마십시오.
                      broken_gaps/connection_mismatches에 함께 기록된 경우 도면 QA 후보이며,
                      법규 위반으로는 별도 확정하지 마십시오.
  ...
  - wall_clearances : MEP 블록과 arch_reference/proxy_wall 간 이격(separation_mm). wall_source로 출처를 구분하며 벽체 이격 규정 판단에 활용.
...
9. 색상, 레이어 기반 도메인 확인
   ...
   layer_role=arch 또는 arch_elements/arch_reference/proxy_wall로 분리된 객체에는 배관 규정을 적용하지 마십시오.
...
위반 보고는 실제 배관 중심선 후보로 남은 handle에 한정하며, 다음 경우는 위반에서 제외하십시오:
  - virtual_connections만으로 추정된 주석 gap
...
{{
  "violations": [
    {{
      ...
      "confidence_score":  "0.0~1.0 사이 신뢰도 (아래 신뢰도 산출 규칙 참고)",
      "confidence_reason": "신뢰도 산출 근거 짧은 한국어 키워드 (예: spec_match_strict, topology_supports, layer_role_unclear)",
      "proposed_action": {{
...
[신뢰도(confidence_score) 산출 규칙]
- 1.00~0.85 : 시방서 수치 규정과 도면 수치가 직접 일치(예: diameter_mm·pressure_mpa 명시), topology가 같은 결론을 뒷받침
- 0.84~0.65 : 규정 텍스트와 도면 데이터가 일치하지만 부속 근거(layer_role, block 속성, 단위) 중 하나가 불완전
- 0.64~0.40 : 규정 인용은 가능하나 수치가 도면에 없거나 관련 객체 식별이 layer 추정에 의존
- 0.40 미만 : violations 에 포함하지 말고 deterministic/QA 검사에 위임
- confidence_reason 은 산출 근거 키워드를 ``,`` 로 구분된 짧은 토큰 형태로 작성
...
def _inject_confidence(violations: list, handle_to_el: dict[str, dict]) -> list:
    """violations 각 항목에 confidence_score / confidence_reason 을 보강합니다.

    LLM이 시스템 프롬프트의 신뢰도 산출 규칙에 따라 직접 부여한 confidence_score 가 있으면
    그 값을 0~1로 clamp해 보존합니다(잘못된 타입/범위는 결정 규칙으로 폴백). LLM이 제공하지
    않은 경우에만 ``_compute_confidence_score`` 결정 규칙으로 산출합니다.
    """
    out = []
    for v in violations or []:
        if not isinstance(v, dict):
        ...
        if provided is not None:
            try:
                ...
                if 0.0 <= score <= 1.0:
                    v["confidence_score"] = round(score, 3)
                    if not v.get("confidence_reason"):
                        v["confidence_reason"] = "llm_self_assessed"
                    valid_provided = True
            except (TypeError, ValueError):
                valid_provided = False
...
_GAS_METER_FIRE_CLEARANCE_RE = re.compile(
    r"(?:가스\s*(?:계량기|미터)|gas\s*meter|meter).*(?:화기|점화|불꽃|fire|ignition)|"
    r"(?:화기|점화|불꽃|fire|ignition).*(?:가스\s*(?:계량기|미터)|gas\s*meter|meter)",
    re.IGNORECASE,
)
_METER_EVIDENCE_RE = re.compile(
    r"계량기|미터|METER|GM\b|GAS\s*METER",
    re.IGNORECASE,
)
_TANK_VIBRATION_CLEARANCE_RE = re.compile(
    r"(?:탱크|저장|용기|tank|vessel|cylinder).*(?:진동|손상|완충|가요|플렉|flex|vibration)|"
    r"(?:진동|손상|완충|가요|플렉|flex|vibration).*(?:탱크|저장|용기|tank|vessel|cylinder)|"
    r"(?:150\s*mm).*(?:탱크|진동|완충|flex|vibration)|"
    r"(?:탱크|진동|완충|flex|vibration).*(?:150\s*mm)",
    re.IGNORECASE,
)
_TANK_VIBRATION_EVIDENCE_RE = re.compile(
    r"탱크|저장|용기|TANK|VESSEL|CYLINDER|가요|플렉|FLEX|EXPANSION|JOINT",
    re.IGNORECASE,
)
_GENERIC_LAYER_RE = re.compile(r"^(?:L\d+|LAYER\d*|\d+)$", re.IGNORECASE)
_SUPPORT_CLAIM_RE = re.compile(
    r"행거|서포트|레스트레인트|브레이스|지지장치|내진\s*지지|"
    r"hanger|support|restraint|brace",
    re.IGNORECASE,
)
_SUPPORT_OBJECT_REQUIRED_DRAWING_TYPES = frozenset({
    "plan",
    "system_diagram",
    "riser",
    "section",
...
_SYMBOL_FRAGMENT_MAX_MM = 300.0
_LINE_LIKE_RAW_TYPES = frozenset({"LINE", "ARC", "POLYLINE", "LWPOLYLINE", "SPLINE"})
_TEXT_RAW_TYPES = frozenset({"TEXT", "MTEXT", "MLEADER", "LEADER"})
_HEIGHT_CLAIM_RE = re.compile(
    r"설치\s*높이|높이|고도|표고|height|elevation|FL\s*[+.\-]|EL\s*[+.\-]|GL\s*[+.\-]",
    re.IGNORECASE,
)
_HEIGHT_EVIDENCE_RE = re.compile(
    r"(?:F\.?\s*L|E\.?\s*L|G\.?\s*L)\s*[+.\-]\s*[\d,]+(?:\.\d+)?|"
    r"[\d,]+(?:\.\d+)?\s*H\b",
    re.IGNORECASE,
)
...
def _has_gas_meter_object_evidence(el: dict | None) -> bool:
    if not isinstance(el, dict):
        return False

    raw = str(el.get("raw_type") or el.get("type") or "").upper()
    if raw in _LINE_LIKE_RAW_TYPES or raw in _TEXT_RAW_TYPES:
        return False

    text = _element_domain_text(el)
    return bool(_GAS_EVIDENCE_RE.search(text) and _METER_EVIDENCE_RE.search(text))
...
def _bbox_max_dim(el: dict) -> float:
    bbox = el.get("bbox")
    if isinstance(bbox, dict):
        try:
            if {"x1", "x2", "y1", "y2"}.issubset(bbox):
                return max(
                    abs(float(bbox["x2"]) - float(bbox["x1"])),
                    abs(float(bbox["y2"]) - float(bbox["y1"])),
                )
            if {"min_x", "max_x", "min_y", "max_y"}.issubset(bbox):
                return max(
                    abs(float(bbox["max_x"]) - float(bbox["min_x"])),
    ...
    for key in ("start", "end", "center", "position", "insert_point"):
        ...
        if p:
            pts.append(p)
    for key in ("vertices", "fit_points"):
        for item in el.get(key) or []:
            p = _xy(item)
            if p:
                pts.append(p)
    if not pts:
...
def _line_like_size_mm(el: dict) -> float:
    candidates: list[float] = []
    try:
        value = el.get("length") or el.get("length_mm")
        if value is not None:
            candidates.append(abs(float(value)))
    except (TypeError, ValueError):
        pass

    start = _xy(el.get("start"))
    end = _xy(el.get("end"))
    if start and end:
        candidates.append(((end[0] - start[0]) ** 2 + (end[1] - start[1]) ** 2) ** 0.5)

    bbox_dim = _bbox_max_dim(el)
    if bbox_dim > 0:
        candidates.append(bbox_dim)

    return max(candidates) if candidates else 0.0


def _is_short_line_symbol_fragment(el: dict | None) -> bool:
    if not isinstance(el, dict):
        return False

    raw = str(el.get("raw_type") or el.get("type") or "").upper()
    if raw not in _LINE_LIKE_RAW_TYPES:
        return False

    size = _line_like_size_mm(el)
    return 0 < size <= _SYMBOL_FRAGMENT_MAX_MM


def _has_tank_vibration_object_evidence(el: dict | None) -> bool:
    if not isinstance(el, dict):
        return False
    return bool(_TANK_VIBRATION_EVIDENCE_RE.search(_element_domain_text(el)))


def _has_explicit_pipe_object_evidence(el: dict | None) -> bool:
    if not isinstance(el, dict):
        return False
    ...
    if material and material not in {"UNKNOWN", "NONE", "NULL"}:
        return True

    attrs = el.get("attributes") or el.get("properties") or {}
    if isinstance(attrs, dict):
        for key in _ATTRS_COMPLIANCE_KEYS:
            value = attrs.get(key) or attrs.get(key.upper())
...
def _has_strong_pipe_target_for_support_claim(el: dict | None) -> bool:
    if not isinstance(el, dict):
        return False
    ...
    connectivity = str(el.get("connectivity_role") or "").lower()
    geometry = str(el.get("geometry_role") or "").lower()

    if geometry == "pipe_symbol" or connectivity in {"symbol", "symbol_fragment"}:
        return False

    if role == "mep" or bool(el.get("flag_for_piping_agent")):
        return True
    if _PIPE_LAYER_RE.search(layer or ""):
        return True
    if _has_explicit_pipe_object_evidence(el):
        return True

    # Ambiguous CAD layers are user-defined. A plain short LINE without
    # entity-level pipe evidence is not enough to create hanger/support issues.
    if raw in _LINE_LIKE_RAW_TYPES and _GENERIC_LAYER_RE.match(layer.strip()):
        return False

    return False


def _has_support_spacing_evidence(v: dict, el: dict | None) -> bool:
    if isinstance(el, dict):
        value = el.get("hanger_spacing_mm")
        if isinstance(value, (int, float)) and value > 0:
            return True
        if isinstance(value, str) and re.search(r"\d", value):
            return True

    current = str(v.get("current_value") or "")
    required = str(v.get("required_value") or "")
    joined = " ".join(
        str(v.get(k) or "")
        for k in ("reference_rule", "reason", "current_value", "required_value")
    )

    has_current_spacing = bool(
        re.search(r"\d+(?:\.\d+)?\s*(?:mm|m)\b", current, re.IGNORECASE)
    )
    has_required_spacing = bool(
        re.search(r"\d+(?:\.\d+)?\s*(?:mm|m)\b", required, re.IGNORECASE)
    )
    support_context = bool(_SUPPORT_CLAIM_RE.search(joined))
    if has_current_spacing and has_required_spacing and support_context:
        return True

    for key in ("related_handles", "support_handles", "hanger_handles"):
        handles = v.get(key)
...
def _support_object_text(el: dict | None) -> str:
    if not isinstance(el, dict):
        return ""
    return _element_domain_text(el)


def _has_support_object_evidence(
    v: dict,
    el: dict | None,
    handle_to_el: dict[str, dict],
) -> bool:
    if _SUPPORT_CLAIM_RE.search(_support_object_text(el)):
        return True
    ...
    for key in ("related_handles", "support_handles", "hanger_handles"):
        ...
        for handle in handles:
            if _SUPPORT_CLAIM_RE.search(_support_object_text(handle_to_el.get(str(handle)))):
                return True
    return False


def _drawing_type_from_context(drawing_context: object) -> str:
    if not isinstance(drawing_context, dict):
        return "unknown"
    drawing_type = str(drawing_context.get("drawing_type") or "").lower()
    if drawing_type:
        return drawing_type

    title = str(drawing_context.get("drawing_title") or "")
    return pipe_drawing_type_from_text(title)


def _requires_support_object_for_claim(drawing_context: object) -> bool:
    return _drawing_type_from_context(drawing_context) in _SUPPORT_OBJECT_REQUIRED_DRAWING_TYPES


def _is_plan_drawing_context(drawing_context: object) -> bool:
    return _drawing_type_from_context(drawing_context) == "plan"


def _is_support_claim(v: dict) -> bool:
    vtype = str(v.get("violation_type") or "")
    if vtype in {
        PipingViolationType.HANGER_SPACING_ERROR.value,
        PipingViolationType.SEISMIC_SUPPORT_ERROR.value,
    }:
        return True
    joined = " ".join(
        str(v.get(k) or "")
    ...
    return bool(_SUPPORT_CLAIM_RE.search(joined))


def _filter_unsupported_gas_meter_fire_clearance(
    violations: list,
    handle_to_el: dict[str, dict],
) -> list:
    """Drop gas-meter/fire clearance claims attached to generic linework.

    The 2 m gas-meter/fire rule needs a gas meter target. On project drawings,
    layer names are often user-defined, so a nearby gas label must not turn a
    polyline symbol/frame into a gas meter clearance violation.
    """
    clean: list = []
    for v in violations or []:
        if not isinstance(v, dict):
        ...
        if not _GAS_METER_FIRE_CLEARANCE_RE.search(joined):
            clean.append(v)
            continue

        eq_id = str(v.get("equipment_id") or "")
        el = handle_to_el.get(eq_id)
        if _has_gas_meter_object_evidence(el):
            clean.append(v)
            continue

        logging.warning(
            "[ComplianceAgent] gas-meter/fire clearance removed: unsupported "
            "target equipment_id=%s raw_type=%s layer=%s",
            eq_id,
            (el or {}).get("raw_type") or (el or {}).get("type"),
            (el or {}).get("layer"),
        )
    return clean


def _filter_unsupported_tank_vibration_claims(
    violations: list,
    handle_to_el: dict[str, dict],
) -> list:
    """Drop tank/vibration clearance claims without explicit tank/flex evidence.

    Reducers, elbows, leader lines, and user-drawn pipe centerlines can be
    ordinary LINE entities. Nearby gas annotations sometimes make the LLM attach
    tank/flexible-joint vibration rules to those lines. Keep this rule only when
    the target object itself carries tank, vessel, cylinder, flexible joint, or
    expansion-joint evidence.
    """
    clean: list = []
    for v in violations or []:
        if not isinstance(v, dict):
        ...
        if not _TANK_VIBRATION_CLEARANCE_RE.search(joined):
            clean.append(v)
            continue

        eq_id = str(v.get("equipment_id") or "")
        el = handle_to_el.get(eq_id)
        if _has_tank_vibration_object_evidence(el):
            clean.append(v)
            continue

        if _is_short_line_symbol_fragment(el):
            logging.warning(
                "[ComplianceAgent] tank/vibration claim removed: unsupported "
                "symbol fragment equipment_id=%s raw_type=%s layer=%s size=%.1f",
                eq_id,
                (el or {}).get("raw_type") or (el or {}).get("type"),
                (el or {}).get("layer"),
                _line_like_size_mm(el or {}),
            )
            continue

        logging.warning(
            "[ComplianceAgent] tank/vibration claim removed: no tank/flex "
            "target evidence equipment_id=%s raw_type=%s layer=%s",
            eq_id,
            (el or {}).get("raw_type") or (el or {}).get("type"),
            (el or {}).get("layer"),
        )
...
def _filter_unsupported_support_claims(
    violations: list,
    handle_to_el: dict[str, dict],
    drawing_context: dict | None = None,
) -> list:
    """Drop hanger/support claims attached to generic linework without pipe evidence."""
    requires_support_object = _requires_support_object_for_claim(drawing_context)
    clean: list = []
    for v in violations or []:
        if not isinstance(v, dict):
            clean.append(v)
            continue
        if not _is_support_claim(v):
            clean.append(v)
            continue

        eq_id = str(v.get("equipment_id") or "")
        el = handle_to_el.get(eq_id)
        if requires_support_object and not _has_support_object_evidence(v, el, handle_to_el):
            logging.warning(
                "[ComplianceAgent] support claim removed: drawing type requires "
                "support object evidence equipment_id=%s drawing_type=%s drawing_title=%r",
                eq_id,
                _drawing_type_from_context(drawing_context),
                (drawing_context or {}).get("drawing_title"),
            )
            continue

        if _has_strong_pipe_target_for_support_claim(el) and _has_support_spacing_evidence(v, el):
            clean.append(v)
            continue

        logging.warning(
            "[ComplianceAgent] support claim removed: unsupported target "
            "equipment_id=%s raw_type=%s layer=%s role=%s",
            eq_id,
            (el or {}).get("raw_type") or (el or {}).get("type"),
            (el or {}).get("layer"),
            (el or {}).get("layer_role"),
        )
    return clean
...
def _is_drawing_quality_violation(v: dict) -> bool:
...
def _filter_text_annotation_compliance_claims(
    violations: list,
    handle_to_el: dict[str, dict],
) -> list:
    """Drop legal/spec claims attached directly to CAD text annotations.

    Pipe TEXT/MTEXT/MLEADER entities are kept as context for labels such as
    "20A", "G", or "3M 가스 계량기". They should enrich nearby pipe geometry or
    drive drawing-quality text checks, but a label itself is not the equipment
    body whose material, clearance, or rating can be regulated.
    """
    clean: list = []
    for v in violations or []:
        if not isinstance(v, dict):
            clean.append(v)
            continue
        if _is_drawing_quality_violation(v):
            clean.append(v)
            continue

        eq_id = str(v.get("equipment_id") or "")
        el = handle_to_el.get(eq_id)
        raw = str((el or {}).get("raw_type") or (el or {}).get("type") or "").upper()
        if raw in _TEXT_RAW_TYPES:
            logging.warning(
                "[ComplianceAgent] TEXT annotation compliance claim removed: "
                "equipment_id=%s violation_type=%s layer=%s text=%r",
                eq_id,
                v.get("violation_type"),
                (el or {}).get("layer"),
                str((el or {}).get("text") or (el or {}).get("content") or "")[:80],
            )
            continue
...
def _element_has_height_evidence(el: dict | None) -> bool:
    if not isinstance(el, dict):
        return False

    for key in ("elevation_mm", "height_mm", "install_height_mm"):
        try:
            if float(el.get(key) or 0) != 0:
                return True
        except (TypeError, ValueError):
            pass
    ...
    text = " ".join(
        str(el.get(key) or "")
        for key in ("text", "content", "name", "block_name")
    )
    attrs = el.get("attributes") or el.get("properties") or {}
    if isinstance(attrs, dict):
        text = " ".join([text, *(str(v) for v in attrs.values() if v is not None)])
    if _HEIGHT_EVIDENCE_RE.search(text):
        return True

    start = el.get("start") or {}
    end = el.get("end") or {}
    try:
        return abs(float(start.get("z") or 0) - float(end.get("z") or 0)) > 1e-9
    except (AttributeError, TypeError, ValueError):
        return False


def _filter_unmeasured_height_claims(
    violations: list,
    handle_to_el: dict[str, dict],
) -> list:
    """Drop installation-height violations when the drawing has no height datum.

    In a 2D plan, a leader/text label such as "3M 가스 계량기" identifies the
    equipment but does not provide installation height. Height claims are kept
    only when the target element itself carries explicit FL/EL/GL/H/elevation
    evidence.
    """
    clean: list = []
    for v in violations or []:
        ...
        if not _HEIGHT_CLAIM_RE.search(joined):
            clean.append(v)
            continue

        eq_id = str(v.get("equipment_id") or "")
        el = handle_to_el.get(eq_id)
        if _element_has_height_evidence(el):
            clean.append(v)
            continue

        logging.warning(
            "[ComplianceAgent] height claim removed: no height evidence "
            "equipment_id=%s raw_type=%s layer=%s",
            eq_id,
            (el or {}).get("raw_type") or (el or {}).get("type"),
            (el or {}).get("layer"),
        )
...
class ComplianceAgent:
    async def check_compliance_parsed(
        ...
        if (
            ...
            result = _filter_text_annotation_compliance_claims(result, _handle_to_el)
            result = _filter_missing_slope_claims(result, _handle_to_el)
            result = _filter_unmeasured_height_claims(result, _handle_to_el)
            result = _filter_unsupported_gas_meter_fire_clearance(result, _handle_to_el)
            result = _filter_unsupported_tank_vibration_claims(result, _handle_to_el)
            result = _filter_unsupported_support_claims(result, _handle_to_el, parsed)
        ...
        filtered = _filter_text_annotation_compliance_claims(filtered, _handle_to_el)
        filtered = _filter_missing_slope_claims(filtered, _handle_to_el)
        filtered = _filter_unmeasured_height_claims(filtered, _handle_to_el)
        filtered = _filter_unsupported_gas_meter_fire_clearance(filtered, _handle_to_el)
        filtered = _filter_unsupported_tank_vibration_claims(filtered, _handle_to_el)
        filtered = _filter_unsupported_support_claims(filtered, _handle_to_el, parsed)
    ...
    async def check_compliance(
        ...
        try:
            ...
            logging.error(
                "[ComplianceAgent] LLM timeout after retries, returning empty violations"
            )
            return []
        except Exception as exc:
            logging.exception("[ComplianceAgent] LLM call failed: %s", exc)
            return []
        ...
        if isinstance(result, dict):
            ...
            return viols if isinstance(viols, list) else []
        if isinstance(result, str):
            if "sLLM" in result or "연결" in result:
                ...
                return []
            d = _repair_and_parse_json(result)
            if d is None:
                logging.getLogger(__name__).warning(
                    "[ComplianceAgent] json repair failed raw_head=%r",
                    (result or "")[:400],
                )
                return []
            if settings.CAD_JSON_DEBUG:
